...
public:
	...
	int cost_rn;
	int getState();
	int score(); //Returns the score of how many moves it is away from a perfect board.
	...
	friend bool operator<(Board a, Board b);  //Return true if board a is less than board b;
	friend bool operator<=(Board a, Board b);  //Return true if board a is less than board b;
	friend std::ostream&  operator<<(std::ostream& ss, const Board& gs);
	...
	void changeState(int new_state); //Changes the state of the board.
	void changeCost(int new_cost); //Ups the cost by one.
